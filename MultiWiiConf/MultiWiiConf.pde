...
void setup() {
  ...
  ControlFont font = new ControlFont(font12);
  controlP5.setFont(font);
  //controlP5.setControlFont(font12);
  //setControlFont(new ControlFont(createFont("Arial bold",12),12));
  addTabs();  
  ...
  //baudListbox.captionLabel().set("BAUD_RATE"); PMK
  ...
  //commListbox.captionLabel().set("PORT COM"); PMK
    ...
    if ((toggleWriteServo || toggleWriteServoLive )&& !toggleRead) {   // MSP_SET_SERVO_CONF
      ...
      if (multiType == AIRPLANE || multiType == PPM_TO_SERVO || multiType == HELI_120_CCPM || multiType == HELI_90_DEG){
        for( i=0;i<8;i++){
          ...
          int q= (int)ServoSliderMIN[i].getValue(); payload.add(char (q % 256) ); payload.add(char (q / 256)  ); //PMK
